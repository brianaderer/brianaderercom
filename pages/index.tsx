...
import { HomeProps, Technology, Headline } from '@/interfaces';
...
const Home: FC<HomeProps> = ({ contacts, technologies, headlines, menu, jobs, projects }) => {
    ...
    return (
        <div className={`p-8 pt-20 w-1/2 overflow-hidden min-h-[100vh]`}>
            <TypeOut
                setSiteVisible={setSiteVisible}
                startProcess={startProcess}
                finishedCallback={finishedCallback}
                firstLineCallback={firstLineCallback}
                strings={typeOutStrings.current}
            />
            <div className={`fixed top-2 right-2 flex flex-col`}>
                {contacts?.map((contact, index) => (
                    <Badge key={index} contact={contact} visible={badgeVisible} />
                ))}
                <div className={`w-full flex flex-col gap-6 justify-center items-end p-8 text-lg ${menuVisible ? `opacity-1 h-auto` : `opacity-0 h-0`} border-b border-l border-green-600 mt-4`}>
                    {menu.map((item, index) => (
                        <MenuLink onClick={() => loadAsset({ target: item.resource })} key={index}>
                            {item.name}
                        </MenuLink>
                    ))}
                </div>
            </div>
            <div className={`${siteVisible && selectedSection === 'skillsAndTech' ? `opacity-1` : `opacity-0 h-0`} mt-10 transition-all`}>
                {flatListToHierarchical(technologies).map(node => printHierarchicalList(node))}
            </div>
            <div className={`${siteVisible && selectedSection === 'cv' ? `opacity-1` : `opacity-0 h-0`} mt-10 transition-all`}>
                {jobs.map((job, index) => (
                    <Job key={index} job={job} />
                ))}
            </div>
            <div className={`${siteVisible && selectedSection === 'projects' ? `opacity-1` : `opacity-0 h-0`} mt-10 transition-all flex flex-col gap-4`}>
